...
import { EventDetail, Ticket, Voucher, categoryOptions, statusOptions } from '../types/types'; // Import Voucher
...
const EventDetailsClient: React.FC<EventDetailsClientProps> = ({ eventData }) => {
    ...
    const [selectedTicketType, setSelectedTicketType] = useState<Ticket | null>(null);
    ...
    const availableVouchers: Voucher[] = useMemo(() => {
        ...
        return [
            { discount: 0.10, startDate: "2024-01-01T00:00:00Z", endDate: "2025-12-31T23:59:59Z" }, // 10% diskon
            { discount: 0.05, startDate: "2025-07-01T00:00:00Z", endDate: "2025-09-30T23:59:59Z" }, // 5% diskon
            { discount: 0.20, startDate: "2025-08-01T00:00:00Z", endDate: "2025-08-10T23:59:59Z" }, // 20% diskon (promo singkat)
        ];
    ...
    useEffect(() => {
        if (!selectedTicketType && eventData.ticketTypes.length > 0) {
            const firstAvailableTicket = eventData.ticketTypes.find(ticket => ticket.stock > 0);
    ...
    const handleConfirmCheckout = () => {
        ...
        console.log("Melanjutkan Checkout:", {
            ...
            quantity: quantity,
            basePrice: basePrice,
            discountApplied: selectedVoucher ? (basePrice * selectedVoucher.discount).toLocaleString('id-ID') : 0,
            voucherUsed: selectedVoucher ? `${(selectedVoucher.discount * 100)}%` : 'None',
            totalPrice: totalPrice
        });

        alert("Proses checkout dilanjutkan! (Ini adalah simulasi)");
        setShowCheckoutModal(false);
        router.push(`/checkout?eventId=${eventData.id}&ticketTypeId=${selectedTicketType.id}&qty=${quantity}&voucher=${selectedVoucher?.discount || ''}`);
    };
    ...
    return (
        <div className="container mx-auto p-0 max-w-6xl font-sans my-5 shadow-xl rounded-xl overflow-hidden bg-white">
            {/* Bagian Banner Event */}
            {eventData.banner && (
                <img src={eventData.banner} alt={eventData.name} className="w-full max-h-[450px] object-cover rounded-t-xl" />
            )}
            ...
            <div className="p-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-8 mb-8 text-gray-700 text-lg">
                    <p><strong>Organizer:</strong> {eventData.organizer || 'N/A'}</p>
                    <p><strong>Lokasi:</strong> {eventData.location}</p>
                    <p><strong>Tanggal:</strong> {formattedStartDate} - {formattedEndDate}</p>
                    <p><strong>Waktu:</strong> {formattedStartTime} - {formattedEndTime} WIB</p>
                    <p><strong>Kategori:</strong> {getCategoryLabel(eventData.category)}</p>
                    <p>
                        <strong>Status:</strong>{" "}
                        <span className={`font-bold ${eventData.eventStatus === 'PUBLISHED' ? 'text-green-600' : eventData.eventStatus === 'CANCELLED' ? 'text-red-600' : 'text-gray-500'}`}>
                ...
                <div className="prose max-w-none text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: eventData.description || '<p>Tidak ada deskripsi untuk event ini.</p>' }} />

                {/* Syarat dan Ketentuan */}
                <h2 className="text-3xl font-semibold mt-10 mb-4 text-gray-800 border-b border-gray-200 pb-2">
                    Syarat dan Ketentuan
                </h2>
                <div className="prose max-w-none text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: eventData.syaratKetentuan || '<p>Tidak ada syarat dan ketentuan khusus.</p>' }} />

                {/* Menu Checkout Event - Pilihan Tiket dalam Card */}
                <hr className="my-12 border-t-2 border-dashed border-gray-300" />
                <h2 className="text-3xl font-semibold mb-6 text-gray-800">Pesan Tiket</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-10">
                    {eventData.ticketTypes.map(ticket => (
                        ...
                        >
                            ...
                            <p className="text-gray-600 text-base mb-3">{ticket.description}</p>
                    ...
                    <div className="bg-gray-100 p-8 rounded-lg shadow-inner flex flex-col items-center gap-6">
                        <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-8 w-full justify-center">
                            ...
                            <div className="flex items-center space-x-4">
                                <label htmlFor="quantity" className="font-bold text-lg text-gray-700">Jumlah Tiket:</label>
                                <input
                                    type="number"
                                    id="quantity"
                                    min="1"
                            ...
                            {validVouchers.length > 0 && (
                                <div className="flex items-center space-x-4">
                                    <label htmlFor="voucher" className="font-bold text-lg text-gray-700">Pilih Voucher:</label>
                                    <select
                                        id="voucher"
            ...
            {showCheckoutModal && selectedTicketType && (
                <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50 p-4">
                    <div className="bg-white p-10 rounded-xl max-w-lg w-full text-center shadow-2xl animate-scale-in">
                        <h3 className="text-3xl font-bold mb-5 text-gray-900">Konfirmasi Pesanan Anda</h3>
                        <p className="mb-3 text-lg text-gray-700">Anda akan memesan:</p>
                        <p className="text-4xl font-extrabold text-blue-700 mb-4">
                            {quantity}x <span className="text-gray-800">{selectedTicketType.name}</span>
                        </p>
                        {selectedVoucher && (
                            <p className="text-lg text-green-600 mb-2">
                                Dengan Voucher: <span className="font-bold">{(selectedVoucher.discount * 100).toFixed(0)}% Diskon</span>
                            </p>
                        )}
                        <p className="mb-6 text-lg text-gray-700">untuk event <strong className="text-blue-600">{eventData.name}</strong></p>
                        {selectedVoucher && basePrice > totalPrice && (
