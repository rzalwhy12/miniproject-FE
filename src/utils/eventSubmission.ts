...
export const submitEvent = async (
  ...
  try {
    ...
    // Debug: log seluruh isi FormData
    ...
    const backendUrl =
      process.env.NEXT_PUBLIC_URL_DATABASE || 'http://localhost:4004';
    const endpoint = `${backendUrl}/event/create`;
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401) {
          ...
          setMessage(
            'Endpoint tidak ditemukan. Pastikan backend server berjalan!'
          );
        } else {
      ...
      if (result.success || result.result?.success || res.status === 201) {
        ...
        setMessage(
          result.message || result.result?.message || 'Gagal membuat event'
        );
      }
    } catch (error: any) {
      ...
      if (error.name === 'TypeError' && error.message.includes('fetch')) {
        setMessage(
          'Tidak dapat terhubung ke server. Pastikan backend server berjalan di ' +
            backendUrl
        );
      } else {
