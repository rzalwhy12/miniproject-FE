            ...
            try {
                ...
                try {
                    ...
                    if (checkoutDataRaw) {
                        ...
                        const eventVouchers = allVouchers.filter((voucher: any) => 
        ...
        return (
            <div className="min-h-screen bg-white flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="text-gray-800 text-xl font-semibold">Loading checkout...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-white py-12 relative overflow-hidden">
            ...
            <div className="container mx-auto max-w-6xl relative z-10 px-4">
                <div className="text-center mb-12">
                    ...
                    <p className="text-gray-600 text-lg mb-6">Complete your purchase with our advanced payment system</p>
                </div>
                            ...
                            {orderSummary.items.map((item, index) => (
                                ...
                                >
                                    <div className="flex justify-between items-center">
                                        <div>
                                            <p className="font-bold text-xl text-gray-800 mb-2 group-hover:text-purple-600 transition-colors">{item.eventName}</p>
                                        ...
                                        {vouchers.map((voucher) => (
                                            ...
                                            >
                                                <div className="relative z-10">
                                                    ...
                                                    <div className="text-gray-600 mb-3 text-sm">{voucher.description}</div>
                                                    ...
                                                    {selectedVoucherId === voucher.id && (
                                                        <div className="mt-3 flex items-center gap-2">
                                                            ...
                                                            <span className="text-purple-600 font-bold text-sm">Selected</span>
                                                        </div>
                                                    )}
                                                </div>
                            ...
                            {userInfo.totalPoint > 0 && (
                                <div className="bg-gray-50 border border-gray-200 p-6 rounded-2xl">
                                    <div className="flex items-center gap-3 mb-4">
                                        ...
                                        />
                                        <label htmlFor="use-points" className="flex-grow text-lg font-bold text-gray-800 flex items-center gap-2">
                                    ...
                                    <div className="ml-8 space-y-2">
                                        <p className="text-gray-600">Available Points: <span className="text-yellow-600 font-bold">{userInfo.totalPoint}</span></p>
                                        {usePoint && (
                    ...
                    <div className="bg-white shadow-xl border border-gray-200 p-8 rounded-3xl hover:shadow-2xl transition-all duration-500 h-fit">
                            ...
                            <div className="bg-gray-50 border border-gray-200 p-4 rounded-xl">
                                <div className="flex items-center gap-3 mb-3">
                                    <input
                                        ...
                                        className="w-5 h-5 rounded accent-purple-600 focus:ring-purple-500 focus:ring-2"
                                    />
                                    <label htmlFor="use-coins" className="text-gray-800 font-semibold flex items-center gap-2">
                                        <svg className="w-5 h-5 text-yellow-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-8c1.11 0 2.08.402 2.599 1M12 8V7m0 1v8m0 0v1m0-1c-1.11 0-2.08-.402-2.599-1" />
                                        </svg>
                                        Bayar dengan Coins
                                ...
                                <p className="text-gray-600 text-sm">
                                    Available: <span className="text-yellow-600 font-bold">{userInfo.totalPoint} coins</span>
                                </p>
                                {usePoint && (
                        ...
                        <div className="space-y-4">
                            <div className="flex justify-between items-center py-3 border-b border-gray-200">
                                ...
                                <span className="text-gray-800 font-bold text-lg">{formatPrice(orderSummary.subtotal)}</span>
                            </div>
                            {orderSummary.voucherDiscount > 0 && (
                                <div className="flex justify-between items-center py-3 border-b border-gray-200">
                                    ...
                                    <span className="text-green-600 font-bold text-lg">-{formatPrice(orderSummary.voucherDiscount)}</span>
                                </div>
                            )}
                            {orderSummary.pointsDiscount > 0 && (
                                <div className="flex justify-between items-center py-3 border-b border-gray-200">
                                    ...
                                    <span className="text-yellow-600 font-bold text-lg">-{formatPrice(orderSummary.pointsDiscount)}</span>
                                </div>
                            )}
                            
                            {/* Summary after discounts */}
                            {(orderSummary.voucherDiscount > 0 || orderSummary.pointsDiscount > 0) && (
                                <div className="bg-gray-50 p-3 rounded-lg border-t border-gray-300">
                                    <div className="flex justify-between items-center text-sm text-gray-600">
                                        <span>Subtotal setelah diskon:</span>
                                        <span className="font-semibold">{formatPrice(orderSummary.subtotal - orderSummary.voucherDiscount - orderSummary.pointsDiscount)}</span>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 p-4 rounded-xl flex flex-col items-center">
                                <span className="text-2xl font-bold text-gray-800 mb-2">Total Pembayaran</span>
                                <span
                                ...
                                >
                                    {orderSummary.total === 0 ? 'GRATIS' : formatPrice(orderSummary.total)}
                                </span>
                                {orderSummary.total === 0 && (
                            ...
                            >
                                <div className="flex items-center justify-center gap-3">
                                    ...
                                    {orderSummary.total === 0 ? 'Confirm Order' : 'Secure Payment'}
                                </div>
                            </button>
                        </div>
                    </div>
